...
Formal foundations **and a working reference implementation** for **dynamic authority confinement** in capability-bearing agent systems: a machine-checked property ("no-escalation", NE), mechanized theorems, an adversarial benchmark suite, an instantiation audit of a production tool server, a running MCP guard that enforces the property, and a draft MCP extension specification.
