...
- **T4 weak form — PROVED (trace theorem), with a recorded caveat.** What was a located gap is now closed. Adding an `issuedHistory` field to the configuration (the accumulated stamps of caretaker messages consumed by `startMsg`) turned the one hard preservation case into a three-line proof: the field holds exactly the stamp the consumed message leaves behind, so the disjunct's witness survives the message's removal from the in-flight set. `T4_weak_trace` proves, over every reachable trace and with no quiescence assumption, that every underlying-only effect is inside `F` or licensed by the issuance history. **Caveat, recorded honestly:** the proved conclusion is *broader* than this note's original prose — it is "`e` was ever issued into history", not the tighter "`e` was in flight when the filter narrowed". `issuedHistory` is a config-level accumulator, not indexed to the narrowing point, so the guarantee is the (weaker) ever-issued-license form. The proof defines the claim; the tighter narrow-time form would need a trace-history-indexed invariant and remains an available sharpening.
