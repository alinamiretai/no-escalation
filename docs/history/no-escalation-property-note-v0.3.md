...
**Status:** working note, **v0.3** — applies `decision-memo-turn-semantics.md` (D1–D4, D-bonus) with D5 amendments (continuation capture; authority-vs-influence; linear resolvers). Frozen for Alloy encoding once §10 items n2–n3 are noted as instantiation-layer text; no further definitional changes without a new decision memo.
...
6. ⛔ **OPEN — never encoded, and encoding it exposes a live design question.** Under the current semantics every effect in a turn carries the turn's π, so a trusted deputy's own-authority internal effect (logging) during a narrow request turn is *flagged*. This is the stack-inspection over-restriction problem; the doPrivileged analog. Requires a decision memo (options: accept the restriction; a self-context privilege for trusted components; scope E to externally observable effects) **before the Lean Guard contract is stated.**
...
Model-driven findings fed back into this note: base case of alias-freedom (§6); the item-6 dual-accounting question (above). Freeze of v1.0 is **conditional on resolving item 6.**
