...
| T4w | **Weak revocation, per-step**: without quiescence, a caretaker's underlying-only effect is `F e ∨ StaleLicensed e` (licensed by a message in flight at narrow-time) | PROVED (per-step) | `Revocation.T4_weak`. *Open:* trace-preservation of `RevInvWeak` without `NoStaleLicense` — one lemma (`RevInvWeak_along`); the located boundary of the weak form |
