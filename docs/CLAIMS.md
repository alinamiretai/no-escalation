...
| T4w | **Weak revocation, per-step**: without quiescence, a caretaker's underlying-only effect is `F e ∨ StaleLicensed e` | PROVED (per-step) | `Revocation.T4_weak` |
| T4w-pres | **Weak-invariant preservation**: `RevInvWeak` survives a step for **8 of 9 constructors** | PROVED (8/9) + located gap | `Revocation.RevInvWeak_step`. Sole open case: `startMsg`, which sets `issued := m.stamp` and *removes* `m` from inflight, so the disjunct's witness is consumed. Closing it needs `Config` to carry issuance history (a consumed message's licensing stamp), or the invariant restated over traces. One `sorry`, marked as the located design obligation |
